...
此前的集成版本只保留了 README 来源说明，没有把 Apache-2.0 许可证副本放进仓库，也没有在被修改的文件中明确标注修改；该副本现已从 `pimono` 删除，改为直接从上游 filtered install。本文不是法律意见。
...
`pimono` 的 `package.json` 当前声明整个 package 的默认许可证为 MIT；如果未来重新复制该第三方文件，必须明确 MIT 只覆盖 `pimono` 自己的代码，并为 Apache-2.0 文件添加 `LICENSES/Apache-2.0.txt` 或等价的第三方许可文件，以及上游地址、许可证和本地修改说明。
...
如果过渡期间同时保留 `pimono` 中的副本和上游 package，必须确保其中一个来源排除 `whimsical`，否则两个入口都会注册 `turn_start` 和 `turn_end`，导致重复加载。现在副本已删除，因此只需让 `pimono` 和上游 package 各自加载自己的扩展集合。
...
本次已采用推荐方案：从 `pimono` 删除 `whimsical` 副本，直接安装上游 package，并在 `settings.json` 只允许 `extensions/whimsical.ts`。如果未来希望所有个人扩展都由 `pimono` 统一锁定和发布，再按前述 Apache-2.0 要求重新集成。
