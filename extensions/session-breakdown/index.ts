 ...
 *
 * Derived from mitsuhiko/agent-stuff/extensions/session-breakdown.ts,
 * licensed under the Apache License, Version 2.0. Modified for pimono:
 * the dashboard and loading view use Pi's full-screen top/bottom border style.
 */
...
export { FULLSCREEN_VIEW_OPTIONS, frameWithPiBorders } from "./layout.ts";
...
export class BreakdownComponent implements Component {
	...
	constructor(data: BreakdownData, tui: TUI, theme: Theme, onDone: () => void) {
		this.data = data;
		this.tui = tui;
		this.theme = theme;
		this.onDone = onDone;
	...
	private tableMaxRows(): number {
		...
		if (rows <= 0) return 12;
		// Fixed chrome above/around the table: Pi frame, header, help, blank,
		// summary, blank, graph (7), blank, table header + divider (2), plus margin.
		const overhead = 21;
	...
	render(width: number): string[] {
		...
		const tab = (days: number, idx: number): string => {
			...
			const label = `${days}d`;
			return selected
				? this.theme.fg("text", this.theme.bold(`[${label}]`))
		...
		const header =
			`${this.theme.fg("accent", this.theme.bold("Session breakdown"))}  ${tab(7, 0)}${tab(30, 1)}${tab(90, 2)}  ` +
		...
		const summary = rangeSummary(range, selectedDays, metric.kind) + this.theme.fg("dim", `   (graph: ${graphDescriptor})`);
		...
		this.cachedLines = frameWithPiBorders(
			lines.map((l) => (visibleWidth(l) > width ? truncateToWidth(l, width) : l)),
			Math.max(1, termRows),
			border,
		);
	...
	pi.registerCommand("session-breakdown", {
		...
		handler: async (_args, ctx: ExtensionContext) => {
			...
			await ctx.ui.custom<void>((tui, theme, _kb, done) => {
				return new BreakdownComponent(data, tui, theme, done);
			}, FULLSCREEN_VIEW_OPTIONS);
		},
	});
}
