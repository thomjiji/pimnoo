...
根 `package.json` 是 umbrella package 的唯一 manifest。`extensions/` 只包含四个运行时 extension 和 `bash-readable` 的内部 helper；`test/`、`scripts/` 和文档不在 `pi.extensions` 的资源范围内。初始版本不发布 npm、不添加第三方 runtime dependency、不创建 sibling package，也不提供会安装其他 package 的 `/setup` 命令。
